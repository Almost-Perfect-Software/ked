...
class TelegramMessenger(BaseMessenger):
    ...
    # ---- Helper telegram API ----
    def _send_message(self, text: str, reply_markup: Dict[str, Any] | None = None):
    ...
    def _edit_message(self, message_id: int, text: str, reply_markup: Dict[str, Any] | None = None):
        text = truncate_text(text, self.TELEGRAM_MSG_MAX_SIZE)
    ...
    def _get_updates(self, offset: int | None = None):
    ...
    def _generate_callback_id(self, data: Dict[str, Any]) -> str:
        callback_id = hashlib.md5(json.dumps(data, sort_keys=True).encode()).hexdigest()[:8]
    ...
    def _build_image_keyboard(self, repo_name: str, images: List[Dict[str, Any]]) -> Dict[str, Any]:
        keyboard = []
        for image in images:
            tags = image.get("imageTags", [])
            for tag in tags:
                if "latest" not in tag.lower():
    ...
    def _handle_service_selection(self, data: str, message: Dict[str, Any], user: Dict[str, Any]) -> None:
        try:
            ...
            if callback_id not in self.callback_data_store:
                self._edit_message(message["message_id"], "❌ Session expired. Please try again.")
                return

            payload = self.callback_data_store[callback_id]
        ...
        try:
            ...
            service_tags = [tag for tag in tags if tag.startswith(service + "-") and "latest" not in tag.lower()]
