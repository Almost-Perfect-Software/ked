...
def filter_tags(tags: List[str]) -> List[str]:
    ...
    """
    return [tag for tag in tags if tag != "<untagged>" and "latest" not in tag]
...
def purge_files(job_config: Dict, download_dir: str, new_helm_arch: str) -> None:
    ...
    # Remove the download directory if it exists and is empty
    if os.path.exists(download_dir):
        try:
            os.rmdir(download_dir)  # Only removes the directory if it's empty
            logger.info(f"Removed empty download directory: {download_dir}")
        except OSError:
            logger.warning(f"Download directory not empty or cannot be deleted: {download_dir}")
