...
from base import ked_pre_deploy, ked_config_parser
...
config = ked_config_parser.load_config()


def deploy(repo_name, tag):
...
def fetch_value_files(repo, helm_project, name, branch, values_file_name, deploy_dir):
    """
    Fetches Helm values files from a remote repository and saves them to a specified directory.
    """
    repository = next((r for r in config.get("repository", []) if r['name'] == repo), None)
    if not repository:
    ...
    try:
        response = requests.get(file_url, auth=HTTPBasicAuth(username, token))
...
def deploy_helm_chart(config, job_config, tag, deploy_dir):
    ...
    helm_name = job_config.get('helm_name')
    ...
    fetch_value_files(helm_values_repo, helm_values_project, helm_name, helm_branch, default_values_file, deploy_dir)
    ...
    for file in values_files:
        ...
        fetch_value_files(helm_values_repo, helm_values_project, helm_name, helm_branch, file, deploy_dir)
...
def dummy_deploy_function(config, repo_name, image_tag):
    ...
    try:
        ...
        deploy_status, deploy_message = deploy(repo_name, image_tag)
